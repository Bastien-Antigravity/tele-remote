...
from os import getpid as osGetpid, name as osName
...
if TELE_REMOTE_SERVER_TYPE.lower() == "async":
    ...
    from asyncio import run as asyncioRun, set_event_loop_policy as asyncioSet_event_loop_policy, \
                        get_running_loop as asyncioGet_running_loop, start_server as asyncioStart_server, \
                        Lock as asyncioLock
    ...
    async def handle_TCP_client(reader, writer):
        ...
        while True:
            try:
                ...
                await current_logger.asyncError("{0} : error while trying to received data, '{1}' is closing connection : {2}".format(Name, clientName, e))
                break              
            if not data:
                break
    ...
    def run_TeleRemote(port=int(getUnusedPort()), ReceiverName:str="TeleRemoteReceiver", conf:str="common", log_level=LOG_LEVEL, only_logger:bool=False):
            ...
            #    current_logger.info("port -> {0}   current_config.TB_PORT -> {1}".format(port, current_config.TB_PORT))
            if osName != 'nt':
                import uvloop
                asyncioSet_event_loop_policy(uvloop.EventLoopPolicy())
            asyncioRun(run_tasks(current_config.TB_PORT, ReceiverName))
...
##############################################################################################################################################################################################
    ...
    class TelecommandSocketServer(ThreadingTCPServer):
        ...
        def __init__(self, config:Config, handler=TeleThreadInterface, port=int(getUnusedPort()), log_level:int=LOG_LEVEL, only_logger:bool=False): 
            ...
            self.config = config
            #if str(self.config.TB_PORT) != str(port) and not config_server:
